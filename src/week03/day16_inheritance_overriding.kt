package week03.day16

fun main() {
    println("Day 16: Inheritance, Overriding")
}

/*
Practice Questions:
1. Explain "Inheritance, Overriding" in your own words with one code example.
2. Write one small program focused on "Inheritance, Overriding".
3. Add one invalid input/edge case and handle it.
4. Improve your code readability (naming + formatting + comments).
5. Test with at least 3 different inputs and note output.
*/
